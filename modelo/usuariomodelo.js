...
module.exports = {
  ...
  delete(id, callback) {
    db.query('DELETE FROM users WHERE id = ?', [id], (err) => {
      ...
      // Llamamos a la función reorderIds después de borrar
  ...
  reorderIds(callback) {
    db.query('SET @count = 0;', (err) => {
      ...
      db.query('UPDATE users SET id = @count := @count + 1;', (err) => {
        ...
        // Obtener el valor máximo del id
        db.query('SELECT MAX(id) AS maxId FROM users', (err, result) => {
          if (err) return callback(err);

          const maxId = result[0].maxId;

          // Ahora actualizar el AUTO_INCREMENT para que el siguiente ID sea el correcto
